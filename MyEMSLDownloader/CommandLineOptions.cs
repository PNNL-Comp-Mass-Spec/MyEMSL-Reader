...
namespace MyEMSLDownloader
{
    internal class CommandLineOptions
    {
        ...
        [Option("DatasetID", HelpShowsDefault = false, HelpText = "Dataset ID", Min = 1)]
        public int DatasetID { get; set; }

        [Option("DataPkg", HelpShowsDefault = false, HelpText = "Retrieve files from a specific data package", Min = 1)]
        public int DataPkgID { get; set; }

        [Option("SubDir", ArgPosition = 2, HelpShowsDefault = false, HelpText = "Name of a subdirectory in the dataset")]
        ...
        /// </remarks>
        [Option("Files", HelpShowsDefault = false, HelpText = "Filter for specific files, for example /Files:*.txt\n" +
        ...
        [Option("FileList", HelpShowsDefault = false, HelpText = "Specify a file with a list of datasets and files to retrieve. " +
        ...
        [Option("FileID", HelpShowsDefault = false, HelpText = "Specify the MyEMSL ID of a file to download (as seen with /V). " +
        ...
        [Option("O", HelpShowsDefault = false, HelpText = "Use to specify an output directory that isn't the current directory")]
        public string OutputDirectoryPath { get; set; }

        [Option("D", HelpShowsDefault = false, HelpText = "Create a directory with the dataset name, then store the files within that directory")]
        public bool MultiDatasetMode { get; set; }

        [Option("Preview", HelpShowsDefault = false, HelpText = "View files that would be downloaded, but not actually download them.")]
        public bool PreviewMode { get; set; }

        [Option("V", "Verbose", HelpShowsDefault = false, HelpText = "Enable verbose preview, showing extended details about each file")]
        public bool VerbosePreview { get; set; }

        [Option("Test", HelpShowsDefault = false, HelpText = "Perform automatic tests using predefined dataset names")]
        public bool AutoTestMode { get; set; }

        [Option("Trace", HelpShowsDefault = false, HelpText = "Display additional debug information")]
        public bool TraceMode { get; set; }

        [Option("UseTest", HelpShowsDefault = false, HelpText = "Connect to test0.my.emsl.pnl.gov instead of my.emsl.pnl.gov")]
